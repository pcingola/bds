...
public abstract class DataRemote extends Data {
    ...
    /**
     * Create an empty URI when parsing fails
     */
    public static URI getEmptyUri(String protocol) {
        try {
            return new URI(protocol + ":///");
        } catch (URISyntaxException e) {
            throw new RuntimeException("Could not parse empty URI for protocol '" + protocol + "'. This should never happen!");
        }
    }
    ...
    protected URI parseUrl(String urlStr, boolean maskErrors) {
        try {
            // No protocol: file
            ...
            if (maskErrors) {
                log("Could not parse URL '" + urlStr + "'");
                return null;
            }
